...
class Csim(KaitaiStruct):
    ...
    class UiFrame(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == 131:
                ...
                self.ax25_info = self._root.BeaconShort(_io__raw_ax25_info, self, self._root)
            elif _on == 272:
                self._raw_ax25_info = self._io.read_bytes_full()
                _io__raw_ax25_info = KaitaiStream(BytesIO(self._raw_ax25_info))
                self.ax25_info = self._root.BeaconLong(_io__raw_ax25_info, self, self._root)
            else:
    ...
    class CallsignRaw(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.callsign_ror = self._root.Callsign(_io__raw_callsign_ror, self, self._root)
